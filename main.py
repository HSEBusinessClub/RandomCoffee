...
conn = psycopg2.connect(host='localhost', user='hse', password='root', database='hse')
cur = conn.cursor()
...
def start(message):
    try:
        ...
        if info is None:
            ...
            cur.execute(f"INSERT INTO users VALUES('{message.from_user.id}', {False}, '{message.from_user.username}',"
            ...
            bot.send_photo(message.chat.id, open('памятка.png', 'rb'), 'Привет!👋\n\nЯ Random Coffee бот Бизнес-клуба'
                                              ...
                                              ' выбранного среди других членов команды Клуба.')
            name(message)

        else:
            if not info[9]:

                cur.execute(f"""UPDATE users SET closed_club={False}, username='{message.from_user.username}', name='{0}', activity='{0}', 
...
def change_form(call: types.CallbackQuery):
    ...
    if call.data == 'change_online':
        cur.execute(f"""UPDATE users SET format='online' WHERE userid={call.message.chat.id}""")
        conn.commit()
    else:
        cur.execute(f"""UPDATE users SET format='offline' WHERE userid={call.message.chat.id}""")
        conn.commit()
...
def save_interests_abd_turn_to_format(message):
    cur.execute(f"""UPDATE users SET interests='{message.text}' WHERE userid={message.from_user.id}""")
    conn.commit()
...
def save(call: types.CallbackQuery):
    ...
    if call.data == 'online':
        ...
        cur.execute(f"""UPDATE users SET format='offline' WHERE userid={call.message.chat.id}""")
        conn.commit()
    cur.execute(f"""UPDATE users SET finished={True} WHERE userid={call.message.chat.id}""")
...
def participate(call: types.CallbackQuery):
    bot.clear_step_handler_by_chat_id(chat_id=call.message.chat.id)
    cur.execute(f"""UPDATE users SET this_week='yes' WHERE userid={call.message.chat.id}""")
    conn.commit()
...
def miss(call: types.CallbackQuery):
    bot.clear_step_handler_by_chat_id(chat_id=call.message.chat.id)
    cur.execute(f"""UPDATE users SET this_week='no' WHERE userid={call.message.chat.id}""")
    conn.commit()
...
def miss_month(call: types.CallbackQuery):
    bot.clear_step_handler_by_chat_id(chat_id=call.message.chat.id)
    cur.execute(f"""UPDATE users SET this_week='month' WHERE userid={call.message.chat.id}""")
    conn.commit()
...
def miss_inf(call: types.CallbackQuery):
    bot.clear_step_handler_by_chat_id(chat_id=call.message.chat.id)
    cur.execute(f"""UPDATE users SET this_week='inf' WHERE userid={call.message.chat.id}""")
    conn.commit()
